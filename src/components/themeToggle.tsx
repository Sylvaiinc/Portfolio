...
export default function	ThemeToggle() {
	...
	useEffect(() => {
		if(dark) {
			...
			localStorage.setItem("data-theme", "luxury")
	...
	return <label className="swap swap-rotate" {...useHoverAnimation("animate-spin")}>
		...
		{/* sun icon */}
		<svg
			className="swap-off h-5 w-5 md:h-7 md:w-7 fill-current hover:fill-info"
		...
		{/* moon icon */}
		<svg
			className="swap-on h-5 w-5 md:h-7 md:w-7 fill-current hover:fill-info"
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 24 24">
			<path
